#!/bin/bash
# Variant A.1: differentiable actor (cross-entropy through the frozen LTM), epoch-13 LTM.
...
LOG=runs_local/a1_diff
CC="0 1"
E13=$CK/cifar_100_subclasses_12_e13.pth
E40=$CK/cifar_100_subclasses_12_e40.pth
STM13=$CK/variants/stm_a1_diff_e13_pt12.pth
STM40=$CK/variants/stm_a1_diff_e40_pt12.pth
COMMON="--actor-training differentiable --coarse-classes $CC --evaluate-steps 800"
...
if [ ! -f "$LOG/pretrain_e13.done" ]; then
  ...
  ( $PY cifar_main_stm_training.py --experiment-type pretrain --fine-classes 1 2 --epochs 12 $COMMON \
      --ltm-checkpoint $E13 --stm-checkpoint $STM13 --run-root runs_a1_diff > "$LOG/pretrain_e13.log" 2>&1 \
...
if [ ! -f "$LOG/pretrain_e40.done" ]; then
  ...
  ( $PY cifar_main_stm_training.py --experiment-type pretrain --fine-classes 1 2 --epochs 12 $COMMON \
      --ltm-checkpoint $E40 --stm-checkpoint $STM40 --run-root runs_a1_diff_e40 > "$LOG/pretrain_e40.log" 2>&1 \
...
for order in "3 4 5" "4 5 3" "5 3 4"; do
  ...
  ( $PY cifar_main_stm_training.py --experiment-type continual --fine-classes $order $COMMON \
      --ltm-checkpoint $E13 --stm-checkpoint $STM13 --run-root runs_a1_diff > "$LOG/continual_$tag.log" 2>&1 \
